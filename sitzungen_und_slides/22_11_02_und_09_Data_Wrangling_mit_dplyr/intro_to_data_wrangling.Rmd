...
- In-depth summarise, mutate und gruou_by die drei wichtigsten Funktionen 
...
deadly_cops %>%
  ...
  mutate(mean_age = round(mean(Alter, na.rm = T), 0), 
...
## Case when - bei mehreren Konditionen
...
cops_datum <- deadly_cops %>%
  ...
  mutate(Datum_aggregiert = case_when(Datum > "2020-01-01" ~ "2020er", 
                                      ...
                                      TRUE ~ "1970er"
                                      #alles, was nicht abgedeckt wurde kann durch "TRUE" ausgewählt werden
                                      )
  ) %>%
  group_by(Datum_aggregiert) %>%
  summarise(Anzahl_Faelle = n())
...
Frage: Wie viele Menschen wurden pro Jahr erschossen? 
